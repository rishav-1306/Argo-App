...
function commandExists(cmd) {
  ...
  try { if (isWindows) { return spawnSync('where', [cmd], { stdio: 'ignore' }).status === 0; } else { return spawnSync('sh', ['-c', `command -v "$1"`, '--', cmd], { stdio: 'ignore' }).status === 0; } } catch { return false; }
}
...
function checkLoginStatus() {
  ...
  try { const r = spawnSync('vercel', ['whoami'], { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], shell: isWindows }); const o = (r.stdout || '').trim(); if (r.status === 0 && o && !o.includes('Error') && !o.includes('not logged in')) { log(`Logged in as: ${o}`); return true; } } catch {}
  return false;
}
